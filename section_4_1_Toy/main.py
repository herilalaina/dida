...
class Toy_data(data.Dataset):
    """ UCI Dataset.
    """

    def __init__(self, set_data):
        name_dataset = os.path.join("/linkhome/rech/genini01/uvp29is/Code/metanal/datasets/toy/", set_data)
...
def load_data(batch_size):
    ...
    data_loader_train = data.DataLoader(dataset=Toy_data(set_data="train"),
                                                          drop_last=True,
                                                          collate_fn=collate_data,
                                                          batch_size=batch_size,
                                                          shuffle=True)
    data_loader_test = data.DataLoader(dataset=Toy_data(set_data="test"),
                                                         drop_last=False,
                                                         collate_fn=collate_data,
                                                         batch_size=batch_size,
                                                         shuffle=False)
